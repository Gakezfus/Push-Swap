...
static int	*search(int *stack, int *sol, int *path, int *depth)
{
	...
	if (path[0] > *depth)
		return (path[0]--, NULL);
	ret = search_2(stack, sol, path, depth);
	if (ret)
		return (path[0]--, ret);
	return (path[0]--, NULL);
}
...
static int	*search_2(int *stack, int *sol, int *path, int *depth)
{
	...
	if (p_dup == NULL || s_dup == NULL ||
		!ft_memcpy(p_dup, path, path[0] * sizeof(int)))
		return (free(s_dup), free(p_dup), *depth = 0, NULL);
	index = 1;
	while(*depth && index < 4)
	{
		...
		ret = search(s_dup, sol, p_dup, depth);
		if (ret)
			return (free(s_dup), free(p_dup), ret);
		index++;
	}
	return (free(s_dup), free(p_dup), NULL);
}
